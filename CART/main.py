...
def bootstrap(data):
    ...
    if isinstance(n, float):
        n = int(n)
    bootstrapRows = []
    # Randomly pick a row from the dataset, with replacement
    for i in range(n):
        randomRow = random.randint(0, len(data) - 1)
        bootstrapRows.append(data[randomRow])
    return bootstrapRows
    # A 2-D list containing bootstrapped samples of the dataset
...
def RFalgorithm(data):
    # Should be around 64-128 trees, odd number preferable
    nTrees = 97
    forest = []
    for i in range(nTrees):
        bootstrapSamples = bootstrap(data)
        tree = DTalgorithm(bootstrapSamples, "RF")
        forest.append(tree)
    return forest
    # A list containing tree dictionaries // NB: when maxDepth is set, some samples could turn out pure &
    # gives "YES"/"NO right away

# Random forest prediction on one row
def RFpredictThis(testRow, forest):
    answerList = []
    for tree in forest:
        # Handling pure samples
        if isinstance(tree, str):
            answerList.append(tree)
        else:
            answerList.append(DTpredictThis(testRow, tree))
    uniqueClasses = list(set([row[-1] for row in dataset]))
    cntClass0 = 0
    cntClass1 = 0
    for item in answerList:
        if item == uniqueClasses[0]:
            cntClass0 += 1
        else:
            cntClass1 += 1
    if max(cntClass0, cntClass1) == cntClass0:
        answer = uniqueClasses[0]
    else:
        answer = uniqueClasses[1]
    return answer

# Random forest prediction on every row of the dataset
def RFprediction(testData, forest):
    cntCorrect = 0
    for testRow in testData:
        answer = RFpredictThis(testRow, forest)
        if answer == testRow[-1]:
            cntCorrect += 1
    return cntCorrect
...
print("[Please wait...]")
forest = RFalgorithm(dataset)
forestAccuracy = getAccuracy(testData, RFprediction(testData, forest))
